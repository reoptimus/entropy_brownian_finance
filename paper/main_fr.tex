...
\paragraph{Deux réserves.} D'abord, les écarts types HAC restent optimistes sur
des fenêtres glissantes qui partagent 251 de leurs 252 jours ; les intervalles
du tableau~\ref{tab:h2} sont à lire comme indicatifs. Ensuite et surtout,
$\beta$ n'a pas encore été confronté aux témoins calibrés de la
section~\ref{sec:verdicts} : le tableau~\ref{tab:placebo} teste la corrélation,
pas la pente. Rien n'exclut qu'un rééchantillonnage produise lui aussi un
$\beta$ positif en régime de stress, pour la même raison mécanique qui lui fait
reproduire H6 --- les fenêtres turbulentes contiennent de grosses journées
communes. \textbf{Tant que ce test n'est pas fait, H2 sous sa forme
\eqref{eq:h2} est une régularité mesurée, pas un résultat établi.} L'ajout de
$\beta$ aux statistiques de \texttt{scripts/placebo\_null.py} est la prochaine
étape.
