...
\paragraph{Proof sketch.} The Lagrangian for normalization, mean and covariance constraints has the form
\[
...
\]
The first-order condition implies $\log p(x)$ is a quadratic function of $x$, so $p$ is Gaussian. Strict concavity of entropy on the feasible set gives uniqueness.
...
\]
This is an empirical hypothesis and is not implied by maximum entropy. In differential notation,
\[
...
\]
The empirical analysis uses finite changes rather than continuous-time differentials.
...
The Gaussian reference distribution is not assumed to equal the empirical distribution. If $q_t$ is the Gaussian distribution matching the empirical mean and covariance and $p_t$ is the empirical density, then
\[
D_{KL}(p_t\Vert q_t)=h(q_t)-h(p_t).
\]
This suggests a useful robustness extension: separate covariance-volume dynamics from deviations from Gaussianity.
